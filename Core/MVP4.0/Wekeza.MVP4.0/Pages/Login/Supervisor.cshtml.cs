...
public class SupervisorModel : PageModel
{
    ...
    private readonly ILogger<SupervisorModel> _logger;
    ...
    public SupervisorModel(IHttpClientFactory httpClientFactory, ILogger<SupervisorModel> logger)
    {
    ...
    public async Task<IActionResult> OnPostAsync()
    {
        ...
        try
        {
            var loginRequest = new LoginRequest { Username = Username, Password = Password, Role = UserRole.Supervisor };
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                if (loginResponse?.Success == true && !string.IsNullOrEmpty(loginResponse.Token))
                {
                    ...
                    HttpContext.Session.SetString("Role", UserRole.Supervisor.ToString());
        ...
        {
            _logger.LogError(ex, "Error during login for Supervisor user {Username}", Username);
